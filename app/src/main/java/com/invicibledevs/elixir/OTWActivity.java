...
public class OTWActivity extends AppCompatActivity implements Runnable {
    ...
    private EditText customerNameEditText, mobileNoEditText, addressEditText, vehicleNoEditText, engineNoEditText, chasisNoEditText, idvEditText, ccEditText, odDiscountEditText,premiumAmountEditText, makeTextView, modelTextView, serviceChargeEditText, financierNameEditText, previousInsuranceNoEditText, emailEditText, pinCodeEditText, vehicleColorEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pinCodeEditText = (EditText) findViewById(R.id.editText_pincode);
        vehicleColorEditText = (EditText) findViewById(R.id.editText_vehicle_color);
...
//        }
    ...
    public static class DatePickerFragment extends DialogFragment
        ...
        public OTWActivity aOTWActivity;
        ...
        public Dialog onCreateDialog(Bundle savedInstanceState) {
            ...
            int month = 0;
            int day = 0;
            Date date = aOTWActivity.getDateFromString("dd/MM/yyyy", aOTWActivity.getRiskDateString());
            if(isDateOfReg)
                date = aOTWActivity.getDateFromString("dd/MM/yyyy", aOTWActivity.getDateOfRegString());
            if(date != null) {
        ...
        public void onDateSet(DatePicker view, int year, int month, int day) {
            if(isDateOfReg)
                aOTWActivity.showDateOfReg(year, month + 1, day);
            else
                aOTWActivity.showRiskStartDate(year, month + 1, day);
        }
    ...
    public void savePolicyDetails(View view)
    {
        ...
        if(customerName.equals("") || mobileNo.equals("") || address.equals("") || pinCode.equals("") || manufactureYear.equals("") || vehicleNo.equals("") || premiumAmount.equals("") || companyName.equals("") || vehicleType.equals("") || engineNo.equals("") || chasisNo.equals("") || dateOfReg.equals("")|| make.equals("") || model.equals("") || vehicleColor.equals("") || idv.equals("")
        ...
        OTW aOtw = new OTW();
        aOtw.setUserId(aUser.getUserId());
        aOtw.setCustomerName(customerName);
        aOtw.setMobileNo(mobileNo);
        aOtw.setAddress(address);
        aOtw.setPinCode(pinCode);
        aOtw.setCompanyName(companyValue + "");
        aOtw.setVehicleType(vehicleTypeValue + "");
        aOtw.setVehicleNo(vehicleNo);
        aOtw.setEngineNo(engineNo);
        aOtw.setChasisNo(chasisNo);
        aOtw.setDateOfReg(dateOfReg);
        aOtw.setMake(make);
        aOtw.setModel(model);
        aOtw.setVehicleColor(vehicleColor);
        if(!idv.equals(""))
            aOtw.setIdv(Double.parseDouble(idv));
        else
            aOtw.setIdv(0);
        aOtw.setCc(cc);

        aOtw.setIsHavingPreviousInsurance(false);
        if(previousInsuranceRadioGroup.getCheckedRadioButtonId() == R.id.yes)
            aOtw.setIsHavingPreviousInsurance(true);
        aOtw.setNcb(ncbValue + "");
        aOtw.setPreviousInsurance(previousInsuranceValue + "");
        if(!odDiscount.equals(""))
            aOtw.setOdDiscount(Integer.parseInt(odDiscount));
        else
            aOtw.setOdDiscount(0);

        if(!premiumAmount.equals(""))
            aOtw.setPreminumAmount(Double.parseDouble(premiumAmount));
        else
            aOtw.setPreminumAmount(0);
        if(!serviceCharge.equals(""))
            aOtw.setServiceCharege(Double.parseDouble(serviceCharge));
        else
            aOtw.setServiceCharege(0);
        aOtw.setRiskStartDate(riskStartDate);
        aOtw.setIsHavingFinance(false);
        if(finaceRadioGroup.getCheckedRadioButtonId() == R.id.finance_yes)
            aOtw.setIsHavingFinance(true);
        aOtw.setFinancierName(finacierName);
        ...
        aOtw.setPaymentType(radioPaymentTypeButton.getText().toString());
        aOtw.setPolicyType(radioPolicyTypeButton.getText().toString());

        aOtw.setYearOfManufacture(manufactureYear);
        aOtw.setPreviousInsuranceNo(previousInsuranceNo);
        aOtw.setEmailId(emailId);

        if(imtRadioGroup.getCheckedRadioButtonId() == R.id.imtYes)
            aOtw.setImtStatus(true);
        if(nilDIPRadioGroup.getCheckedRadioButtonId() == R.id.nilDipYes)
            aOtw.setNilDIPStatus(true);
        if(claimsRadioGroup.getCheckedRadioButtonId() == R.id.claimsYes)
            aOtw.setClaimsStatus(true);
        if(nameTransferRadioGroup.getCheckedRadioButtonId() == R.id.nameTransferYes)
            aOtw.setNameTransferStatus(true);
        theOTW = aOtw;

        progressDialog = ProgressDialog.show(OTWActivity.this,
                "", "Please wait...", true);
    ...
    public void showPolicyPhotosView(View view)
    {
        ...
        mainIntent.putExtra("otwId", theOTW.getOtwId());
        mainIntent.putExtra("isOTW", true);
        int REQUEST_CODE = 123;
        startActivityForResult(mainIntent, REQUEST_CODE);
    }

    public void showPolicyPhotosVehicleView(View view)
    {
        Intent mainIntent = new Intent(this, PhotosPolicyActivity.class);
        mainIntent.putExtra("isOTW", true);
    ...
    {
        if(theOTW != null && theOTW.getOtwId()!= null) {
            confirmAndCloseView();
        }
        else
            finish();
    ...
    public void run()
    {
        if(isInternetConnected())
        {
            ...
            apiHelper.sendOTW(theOTW, aUser, getApplicationContext(), handler);
        }
        else
        {
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            otwId = bundle.getString("otId");
            if(msg.what == 1)
            {
                if(progressDialog.isShowing())
                    progressDialog.dismiss();
                AlertDialog.Builder alt_bld = new AlertDialog.Builder(OTWActivity.this);
                alt_bld.setMessage(responseStatus)
                        .setCancelable(true);
                AlertDialog alert = alt_bld.create();
                alert.setButton(-1,"OK", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        theOTW.setOtwId(otwId);

                        theOTWDataSource.createOTW(theOTW);
    ...
    public class CustomAdapter extends BaseAdapter {
        ...
        public CustomAdapter(OTWActivity otwActivity, JSONArray jsonArray) {
